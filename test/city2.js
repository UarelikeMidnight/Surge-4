...
!(async () => {
	...
	//const city = await $http.get("http://ip-api.com/json/24.48.0.1?fields=country");
	//const jsonData = JSON.parse(data);
	//const country = city.body;
	
	
	$httpClient.get('http://ip-api.com/json/24.48.0.1?fields=country', function (error, response, data) {
    const jsonData = JSON.parse(data);
  });
    ...
    const body = {
        ...
        content: `IP：${ip} \n`
            ...
            + `外部 IP：${externalIP} + ${jsonData.country}`,
